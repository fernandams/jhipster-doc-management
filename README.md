...
Test project for a selection process. Web application for document management developed with JHipster. Allows the addition, viewing, and deletion of documents.
...
During development, Docker was used to facilitate setup. In order to start project (docker installed) follow the commands below:
...
If you have any postgresql service running locally in port 5432 you will need stop that for running the command below (Linux only):

```
sudo service postgresql stop
```

If the backend starts without any errors, you should be able to reach the snapshot screen at http://localhost:8080/.
...
If the frontend starts without any errors, you should be able to reach the server at http://localhost:9000/.
...
# Full Readme Generated by JHispter
